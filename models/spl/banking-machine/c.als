...
featureModel: one FeatureModel
 ...
 fact { all f : FeatureDecision | Multitask in FeatureDecision.dec => Deposit in f.featureModel.feature 
}
 fact { all f : FeatureDecision | Multitask in FeatureDecision.dec => Withdraw in f.featureModel.feature 
}
...
fact {
	all t: Transition | t in DomainModel.transition <=> 
	(t.source in DomainModel.state) and (t.target in DomainModel.state)
}
...
// Constraints to derive DomainModel from Domain decisions
 ...
 fact {
	all d: DomainDecision | 
		((Receipt in d.decision) and (T3 in d.domainModel.transition)) =>
			T5 in d.domainModel.transition
		else T5  not in d.domainModel.transition
}
 fact {
	all d: DomainDecision | 
		((Receipt in d.decision) and (T4 in d.domainModel.transition)) =>
			T6 in d.domainModel.transition
		else T6  not in d.domainModel.transition
}


 fact { all d: DomainDecision | Multitask in d.decision => T3 in d.domainModel.transition }
 fact { all d: DomainDecision | Multitask in d.decision => T2 in d.domainModel.transition }



 fact {all d: DomainModel | (T3 not in d.transition) =>
(T5 not in d.transition)}
 fact {all d: DomainModel | (T4 not in d.transition) =>
(T6 not in d.transition)}
...
assert a {all d: DomainModel | T4 in d.transition}
//check a for 5
run {} for 2
